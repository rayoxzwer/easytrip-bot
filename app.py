...
# ReportLab imports for generating clean PDF documents
...
def create_itinerary_pdf(chat_history: list) -> io.BytesIO:
    """Compiles the conversation summary into a clean, well-formatted printable PDF file."""
    ...
    # Process history logs to build context rows inside the document
...
# =====================================================================
# CONTROLLING PERSONA VIA SYSTEM INSTRUCTIONS
# =====================================================================
SYSTEM_INSTRUCTION = """
You are an empathetic, natural, and efficient travel intake assistant for EasyTrip agency.

CRITICAL PIPELINE RULES:
1. Ask only 1-2 short questions at a time to gather exactly 5 elements: Destination, Budget boundaries, Duration, Vibe, and Diet notes.
2. NEVER output raw python code blocks or structural JSON text to the user screen.
3. ONCE ALL 5 PIECES ARE GATHERED: Present a clean text-based review summary titled exact: "✨ **Final Plan Review**". List out their details cleanly, and ask: "Do you want to make any changes? If not, I can finalize it and build your handmap plan PDF file!"
4. IF THE USER CONFIRMS EVERYTHING LOOKS GOOD (e.g., says "yes", "looks great", "no changes"): Output the exact string "[TRIGGER_PDF]" along with your friendly closing sign-off message. Do not print anything else.
"""
...
async def handle_chat_turn(message: types.Message):
    ...
    try:
        ...
        # Check if the AI confirmed the user gave the green light to finalize
        if "[TRIGGER_PDF]" in ai_response_text:
            # Clean up the system trigger tag before sending text to the user
            ...
            # Generate the document binary on the fly using our utility function
            pdf_data = create_itinerary_pdf(history_rows)
            input_file = BufferedInputFile(pdf_data.read(), filename="EasyTrip_Itinerary.pdf")
            
            # Ship the PDF binary file straight to the user's Telegram screen
            ...
            # Clear database logs for this user to allow starting a fresh journey next time
            conn = sqlite3.connect(DB_FILE)
            cursor = conn.cursor()
            cursor.execute("DELETE FROM chat_history WHERE user_id = ?", (user_id,))
            conn.commit()
            ...
            if ai_response_text:
                ...
                await message.answer("I missed that detail. Where are we heading next?")
